...
const Home = () => {
    ...
    const [alert, setAlert] = useState<IAlert>({ visible: false, color: '', message: '' });
    ...
    useEffect(() => {
        ...
        fetch(process.env.REACT_APP_API_ENDPOINT + '/registrations')
            ...
            .catch(err => setAlert({ visible: true, color: 'danger', message: 'Fehler beim Laden der Registrierungen.' }));
            ...
            <Container>
                <Row>
                    ...
                    >
                        ...
                        {alert.visible && <Alert className="mt-3" color={alert.color}>{alert.message}</Alert>}
                    </Col>
                </Row>
            </Container>
        </div>
